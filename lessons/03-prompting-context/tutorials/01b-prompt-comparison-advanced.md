...
```bash
git init
git add app.py models.py test_app.py
...
```bash
python3 app.py &
curl "http://localhost:5000/todos" -s | python3 -m json.tool
# 테스트 데이터 추가
curl -X POST http://localhost:5000/todos -H "Content-Type: application/json" -d '{"title":"우유 사기"}'
curl -X POST http://localhost:5000/todos -H "Content-Type: application/json" -d '{"title":"빵 사기"}'
curl "http://localhost:5000/todos?q=우유" -s | python3 -m json.tool
kill %1
```
...
curl http://localhost:5000/todos/999 -s | python3 -m json.tool
# {"success": false, "error": "항목을 찾을 수 없습니다."}
kill %1
```
