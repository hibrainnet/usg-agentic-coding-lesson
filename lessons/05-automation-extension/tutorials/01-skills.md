...
본문에는:
- !`cd lessons/05-automation-extension/examples/task-manager && python -m pytest -v 2>&1` 로 테스트 결과 주입
- 전체/통과/실패 수 요약
- 실패 시 원인과 수정 방법 제안
...
```bash
git add lessons/05-automation-extension/examples/task-manager/
```
